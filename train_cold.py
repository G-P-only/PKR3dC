...
import argparse
from tools.metrics import get_cindex, get_rm2
from tools.utils import *
from log.train_logger import TrainLogger
from torch_geometric.data import InMemoryDataset
from preprocessing_aff import create_affinity_graph, affinity_graph_cold, read_data
from model.Pkr3dc_Cold import PKR3dC


class GNNDataset(InMemoryDataset):
    def __init__(self, root, index, types='train', transform=None, pre_transform=None, pre_filter=None, ds=None,
                 cold=None):
        self.ds = ds
        self.cold = cold
    ...
    def raw_file_names(self):
        if self.cold != 'feicold':
            return [self.ds + '_cold_' + self.cold + '_fold_1.csv', self.ds + '_cold_' + self.cold + '_fold_2.csv',
                    self.ds + '_cold_' + self.cold + '_fold_3.csv',
...
def main():
    ...
    parser.add_argument('--dataset', default='kiba/drug', help='task name')
    parser.add_argument('--save_model', action='store_true', help='whether save model or not')
    parser.add_argument('--lr', type=float, default=3e-4, help='learning rate')
    parser.add_argument('--batch_size', type=int, default=32, help='batch_size')
    parser.add_argument('--drug_sim_k', type=int, help='Similarity topk of drug', default=4)
    ...
    if cold == 'feicold':
        affinity_graph, drug_map, drug_weight, protein_map, protein_weight = create_affinity_graph(affinity, ds)
    for i in range(5):
        ...
        if cold != 'feicold':
            pid = np.concatenate([set_list[j].data.pid.numpy().astype(int) for j in range(len(set_list)) if j != i])
            did = np.concatenate([set_list[j].data.did.numpy().astype(int) for j in range(len(set_list)) if j != i])
            affinity_graph, drug_map, drug_weight, protein_map, protein_weight = affinity_graph_cold(affinity, ds, pid, did,
        ...
        model = PKR3dC(affinity_graph=affinity_graph,
                         ag_init_dim=affinity_graph.num_node1s + affinity_graph.num_node2s + 2, dataset=ds, cold=cold).to(device)
        epochs = 3000
        ...
        for _ in range(num_iter):
            ...
            for data in train_loader:
                ...
                if global_step % steps_per_epoch == 0:
                    ...
                    test_loss, test_cindex, test_r2 = test_result(model, test_loader, device, drug_map=drug_map, drug_weight=drug_weight, protein_map=protein_map, protein_weight=protein_weight)
...
def test_result(model, test_loader, device, drug_map, drug_weight, protein_map, protein_weight):
    criterion = nn.MSELoss()
    test_loss, test_cindex, test_r2 = test_val(model, criterion, test_loader, device, drug_map=drug_map, drug_weight=drug_weight, protein_map=protein_map, protein_weight=protein_weight)
    model.train()
    return test_loss, test_cindex, test_r2

def test_val(model, criterion, dataloader, device, drug_map, drug_weight, protein_map, protein_weight):
    model.eval()
    running_loss = AverageMeter()
    pred_list = []
    label_list = []
    for data in dataloader:
        data = data.to(device)
        with torch.no_grad():
            pred = model(data, drug_map=drug_map, drug_weight=drug_weight, protein_map=protein_map, protein_weight=protein_weight)
            loss = criterion(pred.view(-1), data.y.view(-1))
            label = data.y
